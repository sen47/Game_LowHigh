...
const std::int32_t numberMin{ 1 }, numberMax(100);
...
void makeNumber()
{
	char answerRange[3]{ 'l','h','y' };

	int min{ numberMin }, max{ numberMax };
	int counter(7);
	...
	while (true)
	{
		if (counter)
		{
			...
			std::cout << "You guess number " << medium << '\n';
			char answer{ getCharAnswerInRange("If you number is lower(l), if your numver higher(h), if it is your number(y): ",answerRange,sizeof(answerRange) / sizeof(answerRange[0])) };
		...
		{
			std::cout << "You lying to me! I win!\n";
			break;
		}
	}
}
